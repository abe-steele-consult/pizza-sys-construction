...
public class CorpMenu
{
  ...
  {
    boolean wasAdded = false;
    if (branchMenus.contains(aBranchMenus)) { return false; }
  ...
  {  
    ...
    {
      if(index < 0 ) { index = 0; }
      if(index > numberOfBranchMenus()) { index = numberOfBranchMenus() - 1; }
      branchMenus.remove(aBranchMenus);
